...
function fail(res,error){const status=Number(error.statusCode)||500;if(status>=500)console.error('Erro em pedidos:',error);return res.status(status).json({success:false,message:status<500?error.message:'Erro interno ao processar pedido',code:error.code});}
